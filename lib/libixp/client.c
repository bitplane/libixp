...
Version:
	...
	if(!strcmp(fcall.version.version, "unknown")
	&& strcmp(version, IXP_VERSION)) {
		ixp_freefcall(&fcall);
		version = IXP_VERSION;
		goto Version;
	}
	...
	fcall.tattach.aname = "";
	fcall.tattach.n_uname = (uint32_t)getuid();
	if(dofcall(c, &fcall) == 0) {
		ixp_unmount(c);
		return nil;
	}
